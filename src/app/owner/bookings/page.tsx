...
export default function OwnerBookingsPage() {
  ...
  useEffect(() => {
    fetch('/api/owner/bookings').then(r => r.json()).then(j => {
      setBookings(j.data || []);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <DashboardLayout sidebar={<OwnerSidebar />} title="Booking Requests">
      <Card>
        ...
        <CardContent>
          {loading ? (
            <div className="space-y-3">{[...Array(4)].map((_, i) => <div key={i} className="h-20 bg-[#f5f0eb] rounded-xl animate-pulse" />)}</div>
          ) : bookings.length === 0 ? (
            ...
            <div className="space-y-3">
              {bookings.map((b) => (
                <div key={b._id} className="p-4 bg-[#f8f5f1] rounded-xl">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <p className="font-semibold text-dark">{b.userId?.fullName}</p>
                        <Badge variant={statusVariant(b.status)}>{b.status}</Badge>
                      </div>
                      <p className="text-xs text-[#7a7167]">{b.userId?.email}</p>
                      <p className="text-xs text-[#7a7167] mt-1">{b.propertyId?.name} · {b.roomId?.roomType} room</p>
                      <p className="text-xs text-[#7a7167]">Move-in: {new Date(b.moveInDate).toLocaleDateString()} · ₹{b.rent}/mo</p>
                    </div>
                  </div>
                  {b.status === 'pending' && (
                    <div className="flex gap-2 mt-3 ml-14">
                      <Button size="sm" onClick={() => updateStatus(b._id, 'confirmed')}>Confirm</Button>
                      <Button size="sm" variant="danger" onClick={() => updateStatus(b._id, 'cancelled')}>Decline</Button>
                    </div>
                  )}
                </div>
              ))}
