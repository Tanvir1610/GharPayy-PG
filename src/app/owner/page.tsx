...
interface Booking {
  _id: string; status: string; moveInDate: string; rent: number;
  propertyId: { name: string }; roomId: { roomType: string };
...
export default function OwnerDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setStats(s.data || { properties: 0, bookings: 0, occupancy: 0, revenue: 0 });
        ...
        <Card>
          ...
          <CardContent>
            {loading ? (
              ...
              <p className="text-center text-[#7a7167] py-8 text-sm">No booking requests yet.</p>
            ) : (
              <div className="space-y-3">
                {recentBookings.map((b) => (
                  <div key={b._id} className="flex items-center gap-4 p-3 bg-[#f8f5f1] rounded-xl">
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-sm text-dark">{b.userId?.fullName || 'Guest'}</p>
                      <p className="text-xs text-[#7a7167]">{b.propertyId?.name} · {b.roomId?.roomType} room</p>
                      <p className="text-xs text-[#7a7167]">Move-in: {new Date(b.moveInDate).toLocaleDateString()}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="font-bold text-dark text-sm">₹{b.rent}/mo</p>
