...
export default function TenantMessagesPage() {
  ...
  return (
    <DashboardLayout sidebar={<TenantSidebar />} title="Messages">
              ...
              <div key={m.id} className={`flex gap-3 py-4 ${!m.is_read&&!isMine?'bg-brand/5 -mx-5 px-5 rounded-xl':''}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <p className="font-semibold text-sm text-dark">{other?.full_name}</p>
                    <span className="text-xs text-[#7a7167] shrink-0">{new Date(m.created_at).toLocaleDateString()}</span>
                  </div>
                  {m.property && <p className="text-xs text-brand mb-0.5">Re: {m.property.name}</p>}
