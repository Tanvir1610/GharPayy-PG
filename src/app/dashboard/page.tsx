...
interface Booking { id: string; status: string; move_in_date: string; rent: number; property: { name: string; city: string; photos?: string[] } }
  ...
  useEffect(() => {
    Promise.all([
      ...
      setStats({
        ...
        messages: (m.data || []).filter((x: { is_read: boolean; receiver_id: string }) => !x.is_read && x.receiver_id !== user?.id).length,
      });
    ...
    <DashboardLayout sidebar={<TenantSidebar />} title="My Dashboard">
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="h-14 bg-[#f5f0eb] rounded-lg animate-pulse" />)}</div>
            : bookings.length === 0 ? <p className="text-center text-[#7a7167] py-8 text-sm">No bookings yet. <Link href="/search" className="text-brand">Browse PGs →</Link></p>
            : (
              <div className="space-y-3">
                {bookings.map(b => (
                  <div key={b.id} className="flex items-center gap-3 p-3 bg-[#f8f5f1] rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-dark text-sm truncate">{b.property?.name}</p>
