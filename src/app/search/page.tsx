...
interface Property {
  ...
  photos: string[]; amenities: string[]; average_rating: number;
  total_reviews: number; gender_preference: string; is_verified: boolean;
}
      ...
      <div className="bg-white border-b border-[#e8e2d8] sticky top-16 z-20">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <form onSubmit={handleSearch} className="flex flex-wrap gap-2 items-end">
            ...
            <Button type="submit" size="sm"><Filter size={14} /> Filter</Button>
          </form>
        </div>
      </div>
      <main className="flex-1 max-w-6xl mx-auto px-4 py-8 w-full">
        <p className="text-sm text-[#7a7167] mb-6">{loading ? 'Searching…' : `${properties.length} properties found`}</p>
        {loading ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[...Array(6)].map((_, i) => <div key={i} className="rounded-2xl bg-[#e8e2d8] animate-pulse h-64" />)}
          </div>
        ) : properties.length === 0 ? (
                ...
                <Card className="overflow-hidden hover:shadow-md transition-shadow h-full">
                  <div className="h-44 bg-[#f5f0eb] relative overflow-hidden">
                    {p.photos?.[0] ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-[#b0a99f] text-4xl font-bold">{p.name[0]}</div>
                    )}
                    {p.is_verified && <span className="absolute top-2 left-2 bg-green-600 text-white text-xs font-semibold px-2 py-0.5 rounded-full">Verified</span>}
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-xs text-[#7a7167] flex items-center gap-1 mb-2"><MapPin size={11} />{p.city}</p>
                    <div className="flex items-center justify-between">
                      <div className="flex gap-1 flex-wrap">
                        {p.amenities?.slice(0, 2).map(a => <span key={a} className="text-xs bg-[#f5f0eb] text-[#7a7167] px-2 py-0.5 rounded-full">{a}</span>)}
                      </div>
                      <div className="flex items-center gap-1 text-xs text-[#7a7167] shrink-0">
                        <Star size={11} className="fill-brand text-brand" />{p.average_rating?.toFixed(1) || 'New'}
                      </div>
                    </div>
                  </div>
                </Card>
