...
const FEATURED_SLUGS = ['korve-coed-k', 'wyse-girls', 'koravill-girls', 'homely-girls', 'nex-coed', 'zexus-boys'];
const BASE = 'https://gharpayy.com/xAssets/cards/koramangala';
const FEATURED = [
  { slug:'korve-coed-k',    name:'Korve Coed Koramangala',       gender:'Co-ed',      tier:'Basic Plus', rent:18000, rating:4.8, reviews:54, photo:`${BASE}/korvek/1.jpg`,         address:'Koramangala' },
  { slug:'wyse-girls',     name:'Wyse Girls Koramangala',        gender:'Girls Only', tier:'Basic Plus', rent:19000, rating:4.8, reviews:63, photo:`${BASE}/wysegirlk/5.jpg`,      address:'Koramangala' },
  { slug:'koravill-girls', name:'Koravill Girls Ejipura',        gender:'Girls Only', tier:'Basic Plus', rent:11000, rating:4.9, reviews:82, photo:`${BASE}/koravillgirls/1.jpg`,  address:'Ejipura, Koramangala' },
  { slug:'homely-girls',   name:'Homely Girls Koramangala',      gender:'Girls Only', tier:'Classics',   rent:17000, rating:4.9, reviews:78, photo:`${BASE}/homelygirls/1.png`,    address:'Koramangala' },
  { slug:'nex-coed',       name:'Nex Coed Koramangala',          gender:'Co-ed',      tier:'Basic Plus', rent:13000, rating:4.7, reviews:59, photo:`${BASE}/nexcoedk/1.jpg`,       address:'Koramangala' },
  { slug:'zexus-boys',     name:'Zexus Boys Koramangala',        gender:'Boys Only',  tier:'Premium',    rent:16000, rating:4.8, reviews:41, photo:`${BASE}/zexusboys/1.jpg`,      address:'SG Palya, Koramangala' },
];
...
const TESTIMONIALS = [
  ...
  { name:'Sneha K.',  role:'Product Manager',        text:'Love the premium tier — the co-working space and amenities are a game changer. Worth every rupee!',                                 rating:5, city:'Whitefield' },
];

const POPULAR_AREAS = [
  { area: 'Koramangala', sub: `${PROPERTY_STATS.koramangala}+ PGs`, href: '/koramangala', color: 'from-amber-500/20 to-orange-400/10' },
  { area: 'Bellandur',   sub: 'Near Ecoworld & Flipkart',       href: '/bellandur',      color: 'from-blue-500/10 to-indigo-400/10' },
  { area: 'Whitefield',  sub: 'Near ITPL & Hopefarm Metro',     href: '/whitefield',     color: 'from-purple-500/10 to-pink-400/10' },
  { area: 'Marathahalli',sub: 'Near AECS & ORR',                href: '/marathahalli',   color: 'from-green-500/10 to-emerald-400/10' },
  { area: 'HSR Layout',  sub: 'Near Bommanahalli Metro',        href: '/hsr-layout',     color: 'from-rose-500/10 to-red-400/10' },
  { area: 'Electronic City', sub: 'Near Infosys & TCS',         href: '/electronic-city',color: 'from-cyan-500/10 to-sky-400/10' },
  { area: 'BTM Layout',  sub: 'Near Silk Board Metro',          href: '/btm-layout',     color: 'from-teal-500/10 to-emerald-400/10' },
  { area: 'All Areas',   sub: `${PROPERTY_STATS.total}+ total PGs`,href: '/search',      color: 'from-brand/20 to-amber-400/10' },
];
    ...
    <div className="min-h-screen flex flex-col bg-[#fdf9f4]">
      ...
      >
        ...
        <div className="relative max-w-3xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-brand/20 text-brand text-xs font-semibold px-3 py-1.5 rounded-full mb-6 border border-brand/30">
            <MapPin size={12} />Bangalore — {ALL_AREAS.length}+ Areas
          </div>
          <h1 className="font-display text-4xl sm:text-5xl md:text-6xl font-bold text-white leading-tight mb-5">
            Find Your Perfect<br />
            <span className="text-brand">PG in Bangalore</span>
          </h1>
          <p className="text-white/60 text-lg mb-8 max-w-xl mx-auto">
            {PROPERTY_STATS.total}+ verified PGs across Koramangala, Bellandur, Whitefield & 10 more areas.
            Fully furnished · Meals included · Starting ₹8K/mo
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center max-w-lg mx-auto mb-8">
            <Link href="/search"
              ...
              <Search size={16} />Browse All {PROPERTY_STATS.total}+ PGs
            </Link>
      ...
      <div className="bg-white border-b border-[#e8e2d8]">
        <div className="max-w-4xl mx-auto px-4 py-5 grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { icon: Home,       value: `${PROPERTY_STATS.total}+`, label: 'Verified PGs' },
            { icon: Users,      value: '1,500+',                   label: 'Happy Tenants' },
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-end justify-between mb-8">
            <div>
              ...
              <h2 className="font-display text-2xl sm:text-3xl font-bold text-dark">Top PGs in Koramangala</h2>
              <p className="text-[#7a7167] text-sm mt-1">Hand-picked, verified & loved by tenants</p>
            </div>
            <Link href="/search" className="hidden sm:flex items-center gap-1 text-sm font-semibold text-brand hover:text-brand-dark no-underline transition-colors">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURED.map(p => (
              <Link key={p.slug} href={`/property/${p.slug}`} className="no-underline group">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-1.5">
                        ...
                        <span className="bg-brand hover:bg-brand-dark text-white text-[11px] font-semibold px-2.5 py-1.5 rounded-xl cursor-pointer">
                          View →
                        </span>
                      </div>
                    </div>
          ...
          <div className="text-center mt-8">
            <Link href="/search"
              ...
              View All {PROPERTY_STATS.total}+ Properties <ArrowRight size={15} />
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <h2 className="font-display text-2xl sm:text-3xl font-bold text-dark mb-2">All amenities, zero hassle</h2>
      ...
      <section className="py-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="text-[#7a7167] text-sm mt-2">{ALL_AREAS.length}+ areas covered across Bangalore</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {POPULAR_AREAS.map(({ area, sub, href, color }) => (
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {[
              ...
              <div key={step} className="relative flex flex-col items-center text-center p-6 bg-[#f8f5f1] rounded-2xl border border-[#e8e2d8] hover:shadow-md transition-shadow">
      ...
      <section className="py-16 px-4 bg-[#f8f5f1]">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className="font-display text-2xl sm:text-3xl font-bold text-dark">Loved by 1,500+ tenants</h2>
          </div>
        ...
        <div className="max-w-xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link href="/search"
              ...
              <Search size={16} />Browse {PROPERTY_STATS.total}+ PGs
            </Link>
          </div>
        </div>
      </section>
