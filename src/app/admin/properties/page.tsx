...
interface Property {
  _id: string; name: string; city: string; address: string; genderPreference: string;
  isVerified: boolean; createdAt: string; amenities: string[];
...
export default function AdminPropertiesPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/properties').then(r => r.json()).then(j => {
  ...
  const toggleVerify = async (propertyId: string, isVerified: boolean) => {
    const res = await fetch('/api/admin/properties', {
      ...
      body: JSON.stringify({ propertyId, isVerified: !isVerified }),
    });
    if (res.ok) {
      setProperties(prev => prev.map(p => p._id === propertyId ? { ...p, isVerified: !isVerified } : p));
      toast({ title: `Property ${!isVerified ? 'verified' : 'unverified'}`, variant: 'success' });
    } else {
  ...
  const filtered = properties.filter(p =>
    ...
    p.ownerId?.email?.toLowerCase().includes(search.toLowerCase())
  );
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-3 py-2 rounded-lg border border-[#e8e2d8] text-sm bg-[#fdf9f4] focus:outline-none focus:border-brand w-60"
              />
            </div>
          </div>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-[#7a7167]">
              <Building2 size={40} className="mx-auto mb-3 text-[#b0a99f]" />
              <p className="font-medium">No properties found</p>
            </div>
          ) : (
            <div className="space-y-3">
              {filtered.map((p) => (
                <div key={p._id} className="flex items-start gap-4 p-4 bg-[#f8f5f1] rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#7a7167] flex items-center gap-1 mt-0.5">
                      <MapPin size={10} /> {p.city} — {p.address}
                    </p>
                    <p className="text-xs text-[#7a7167] mt-0.5">
                      Owner: {p.ownerId?.fullName} ({p.ownerId?.email})
                    </p>
                  ...
                  <div className="shrink-0">
                    <Button
                      ...
                      onClick={() => toggleVerify(p._id, p.isVerified)}
                      className="flex items-center gap-1.5"
                    >
                      {p.isVerified
                        ? <><ShieldOff size={13} /> Unverify</>
